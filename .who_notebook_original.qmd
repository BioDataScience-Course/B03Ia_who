...
<!--# Vous noterez qu'il est bizarre d'utiliser la mortalité moyenne adulte pour prédire l'espérance de vie car les deux sont issues des mêmes données au départ, soit les recensements de mortalités dans le pays. Donc si on a ce qu'il faut à disposition pour en calculer la moyenne, on peut aussi très bien calculer précisément l'espérance de vie... et le modèle est inutile. Cependant, le modèle fait plus que cela, il permet de voir comment chaque variable intervient dans la prédiction de l'espérance de vie, ce qui nous donne des informations utiles sur l'impact de ces variables. C'est dans cette optique-là que notre modèle est élaboré. -->
...
[] -   Les nuages de points ont des étalements similaire en fonction des trois variables, mais l'association est positive pour la mortalité aalors qu'elle est inverse pour l'IMC et la scolarité par rapport à l'espérance de vie.
[] -   Le nuage de points montre une association positive forte entre la mortalité adulte et l'espérance de vie pour tous les pays.
[] -   Les nuages de points pour l'IMC et la scolarité indiquent une association inverse et moyenne avec l'espérance de vie, aussi bien pour les pays non développés que pour les pays développés.
[] -   Les nuages de points pour l'IMC et la scolarité indiquent une association inverse et moyenne avec l'espérance de vie pour les pays non développés. L'association est inexistante ou très faible pour les pays développés.}-")
```
...
<!--% Calculez un second modèle simplifié en fonction de vos observations du premier. Vous placez le résultat dans `who00_lm2` et en ferez le résumé uniquement. Vous simplifierez le modèle en éliminant les interactions entre mortalité et l'espérance de vie et entre l'IMC et l'espérance de vie (pensez à la commutativité des opérateurs dans la formule et aidez-vous de ce qui est déjà prérempli). -->
